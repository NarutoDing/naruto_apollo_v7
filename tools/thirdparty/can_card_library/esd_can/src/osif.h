...
#else
# define OSIF_MALLOC(s,p)                  osif_malloc((UINT32)(s),(VOID**)(p)) /*!< see osif_malloc(...) */
...
#define OSIF_PRINT(fmt)                    (void)osif_print fmt
#define OSIF_DPRINT(fmt)                   (void)osif_dprint fmt
...
typedef INT32 (OSIF_CALLTYPE *OSIF_PFN_TIMER_CREATE)( OSIF_TIMER *t, VOID (OSIF_CALLTYPE *func)(VOID *), VOID *arg, VOID *pCanNode );
...
#define OSIF_TIMER_DESTROY(t)              (void)osif_callbacks.timer_destroy(t)
#define OSIF_TIMER_SET(t,m)                osif_callbacks.timer_set((t),(m))
#define OSIF_TIMER_GET(t,m)                (void)osif_callbacks.timer_get((t),(m))
...
#define OSIF_SHIFT_R_SIGNED(x,sft) (((x)>=0)?((x)>>(sft)):(-((-(x))>>(sft)))) /* warning, x is evaluted 2 times! */

#define OSIF_SER_OUT(str)                  osif_ser_out((str))

#define OSIF_MAKE_NTCAN_SERIAL(pszSerial, pulSerial) \
        (void)osif_make_ntcan_serial(pszSerial, pulSerial)
...
/*
 * Macro to return the absolute value of a signed integer.
 */
#define OSIF_ABS(x) (((x)<0) ? -(x) : (x))
...
/*
 * OSIF_RCSID() is used to insert a version tag in the binary with support to
 * prevent compiler warnings about unused variables for different compiler and
 * support to tame the optimizer which are eager to remove unused static
 * variables in the final build.
 */
 #if defined(_MSC_VER) && (_MSC_VER >= 800) /* MSC compiler */
 # define OSIF_RCSID(msg) \
     static const char* const rcsid[] = {(char *)rcsid, msg}
 #elif ((__GNUC__ > 3) || (__GNUC__ == 3 && __GNUC_MINOR__ >=1)) /* GCC >= 3.1 */
 # define OSIF_RCSID(msg) \
     static const char* const rcsid[] __attribute__((unused,used)) = \
         {(char *)rcsid, msg}
 #elif ((__GNUC__ > 2) || (__GNUC__ == 2 && __GNUC_MINOR__ >=7)) /* GCC >= 2.7 */
 # define OSIF_RCSID(msg) \
     static const char* const rcsid[] __attribute__((unused)) = \
         {(char *)rcsid, msg}
 #else /* No or old GNU compiler */
 # define OSIF_RCSID(msg)                                              \
     static void use_rcsid(void *x) {                             \
         static const char* const rcsid[] = {(char *)rcsid, msg}; \
         if(x) use_rcsid((void *)rcsid);}
